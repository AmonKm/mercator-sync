...
class VCenterConnector(BaseConnector):
    ...
    def authenticate(self) -> None:
        ...
        requête = requests.post(
            f"{self.config['base_url']}/api/session",
    ...
    def fetch_clusters(self) -> list[dict]:
        requête = requests.get(
            f"{self.config['base_url']}/api/vcenter/cluster",
    ...
    def fetch_vms(self, cluster_id: str) -> list[dict]: # On parcours l'ensemble des vms PAR cluster
        requête = requests.get(
            f"{self.config['base_url']}/api/vcenter/vm",
    ...
    def enrich_vm(self, vm_id: str, vm: dict) -> dict:
        ...
        r_details = requests.get(
            f"{self.config['base_url']}/api/vcenter/vm/{vm_id}",
            headers=self.headers, verify=self.verify
        )
        ...
        r_guest = requests.get(
            f"{self.config['base_url']}/api/vcenter/vm/{vm_id}/guest/identity",
            headers=self.headers, verify=self.verify
        )
    ...
    def build_vm_payload(self, vm_id: str, enriched: dict) -> dict:
        ...
        ip      = enriched.get("guest", {}).get("ip_address", "") # A revoir, utiliser MainIP....
        info    = enriched.get("guest", {}).get("full_name", {}).get("default_message", "")

        return {
            "name":             enriched.get("identity", {}).get("name", ""),
            "description":      f"VM importée du Vcenter de ... ({vm_id})<br>{enriched.get('name_description', '')}",
            "operating_system": info,
            "address_ip":       ip,
            "cpu":              cpu,
            "memory":           mem_go,
            "attributes":       f"vcenter_id:{vm_id}", # A modifier si l'on veut faire un ajout
        }
    
    def build_cluster_payload(self, cluster_id: str, cluster: dict) -> dict:
        return {
            "name":       cluster.get("name", ""),
            "attributes": f"vcenter_id:{cluster_id}",
        }
